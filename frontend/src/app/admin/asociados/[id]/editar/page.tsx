...
import type { Asociado, ActualizarAsociadoDto } from '@/asociados/tipos/asociado.tipos';
...
export default function EditarAsociadoPage() {
  ...
  const [formulario, setFormulario] = useState<ActualizarAsociadoDto>({
    nombre: '',
    cargo: '',
    empresa: '',
    descripcion: '',
  ...
  useEffect(() => {
    if (!estaCargando) {
      if (!estaAutenticado) {
        router.push(`/login?returnUrl=/admin/asociados/${id}/editar`);
  ...
  }, [estaAutenticado, estaCargando, usuario, router, id]);

  const cargarAsociado = async () => {
    try {
      const data = await asociadosServicio.obtenerAsociado(id);
      setAsociado(data);
      setFormulario({
        nombre: data.nombre,
        cargo: data.cargo,
      ...
      setError('Error al cargar el asociado');
      console.error('Error al cargar asociado:', err);
    } finally {
      setCargando(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await asociadosServicio.actualizarAsociado(id, formulario, fotoFile || undefined);
      router.push('/admin/asociados');
    } catch (err) {
      setError('Error al actualizar el asociado. Por favor intenta de nuevo.');
  ...
  if (!asociado) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-50">
        <div className="text-center">
          <h2 className="text-xl font-bold text-zinc-900 mb-2">Asociado no encontrado</h2>
          <Link href="/admin/asociados" className="text-zinc-600 hover:text-zinc-900">
            Volver a la lista
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-zinc-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-zinc-900">Editar Asociado</h1>
          <p className="text-zinc-600 mt-2">
            Editando: {asociado.nombre}
          </p>
        </div>
